...
- `src-tauri/`: Tauri Rust backend.
  ...
  - `src-tauri/src/settings.rs`: Settings backend (`AppSettings` struct with `#[derive(TS)]` and schema definition emitting `src/lib/bindings/AppSettings.generated.ts`). `get_settings` merges stored rows over the defaults with a serde round-trip per field (a wrongly-typed stored value keeps the default), and `update_setting` persists one typed setting. Arbitrary per-component UI state lives in the separate `ui_state` table via `get_ui_state`/`set_ui_state`; `settings` holds only `AppSettings` fields.
