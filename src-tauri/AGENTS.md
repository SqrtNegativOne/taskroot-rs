...
- **Database & Migrations**: All tasks and events are stored locally in an SQLite database (`taskroot.db`) located in the app data directory. The schema is created in `db::init_db` (`src-tauri/src/db/migrations.rs`) using inline SQL queries with `CREATE TABLE IF NOT EXISTS` guards, followed by additive `ensure_column!` migrations for columns added after a database was first created (`events.timezone`, `calendars.sync_token`, `tasks.task_list_id`). The legacy `ui.*` → `ui_state` copy runs inside a transaction so the copy and delete cannot interleave with a crash. `db/mod.rs` only wires submodules and `FilterColumnExt`; `db/tests.rs` holds the backend CRUD/migration tests.
