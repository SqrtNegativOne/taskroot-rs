...
//!
//! This module is split into [`storage`] (value decode/encode and the merge rule)
//! and [`metadata`] (schema assembly); command handlers stay here.
...
pub async fn get_settings(app: tauri::AppHandle) -> Result<AppSettings, AppError> {
    ...
    let stored = storage::load_stored_settings(&pool).await?;
    Ok(storage::apply_stored_settings(stored))
}
...
pub async fn update_setting(
    ...
    storage::ensure_valid_setting_value(&key, &value)?;
    let pool = crate::db_pool(&app)?;
    let value_str = storage::encode_stored_value(&value)?;
...
pub async fn get_ui_state(app: tauri::AppHandle, key: String) -> Result<Value, AppError> {
    ...
    Ok(storage::parse_setting_value(raw.as_deref()))
}
