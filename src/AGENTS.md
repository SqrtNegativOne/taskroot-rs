...
- **Persistence Channels**: Backend SQLite is the source of truth for anything that must survive a webview data reset or be shared across windows. Typed app settings are `AppSettings` fields written with `update_setting`; arbitrary per-component UI state goes through `get_ui_state`/`set_ui_state` in the `ui_state` table. The rule is that `localStorage` is not used for user-facing state. **P4 pending:** `src/screens/sidebar/Sidebar.svelte` still uses `localStorage` for `sidebar_tab_top`/`sidebar_notes`/`sidebar_show_notes`; P4 migrates them to the backend (seeding from the existing `localStorage` values on first run). Until then this rule is aspirational for that one component. (`src/routes/dev/+page.svelte` is the dev inspector and is exempt.)
