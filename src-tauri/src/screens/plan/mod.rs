...
fn parse_iso_to_mins(iso: &str, day_start_prefix: &str) -> Option<f64> {
    ...
    if let Ok(dt) = NaiveDateTime::parse_from_str(iso, "%Y-%m-%dT%H:%M:%S") {
        ...
        let time = dt.time();
        return Some((time.format("%H").to_string().parse::<f64>().unwrap_or(0.0) * 60.0) + time.format("%M").to_string().parse::<f64>().unwrap_or(0.0));
    }
    // Try without seconds
...
fn layout_day_events(date_str: &str, events: &[AppEvent]) -> Vec<LaidEvent> {
    let mut mapped: Vec<LaidEvent> = events.iter().filter_map(|e| {
        ...
        if end_iso < &format!("{}T00:00:00", date_str) || start_iso > &format!("{}T23:59:59", date_str) {
            return None;
        }
    ...
    for ev in mapped.iter_mut() {
        ...
        loop {
            let conflict = placed.iter().any(|p| {
                p.2 == lane && !(p.1 <= ev.start_mins || p.0 >= ev.end_mins)
            });
            if !conflict {
                break;
            }
...
pub async fn get_filtered_tasks(
    ...
    if !filters.is_empty() {
        tasks.retain(|t| {
            ...
            for f in &filters {
                ...
                let is_keep = if op == "is not" { !match_found } else { match_found };
                if !is_keep {
                    return false;
                }
                }
    ...
    if !query.trim().is_empty() {
        ...
        tasks.retain(|t| {
            ...
            t.tags.as_ref().map_or(false, |tags| tags.iter().any(|tag| tag.to_lowercase().contains(&q)))
        });
    }
