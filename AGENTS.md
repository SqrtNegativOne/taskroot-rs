...
- `src/`: SvelteKit frontend codebase.
  - `src/routes/`: SvelteKit routing (`+layout.svelte`, `+page.svelte`). The single SPA branches on the Tauri window label (`main`, `launcher`, `minitracker`, `sidebar`, `effect`) inside `+layout.svelte`. `src/routes/effect/` is the transparent, click-through full-screen phase-transition overlay.
  ...
  - `src/screens/`: Major UI views. `plan/` (with `day-timeline/`, including `hooks/pointerGesture.svelte.ts` — a window-pointer gesture registry with `pointercancel` and teardown safety — and `date-grid/`, plus `hooks/eventDrag.svelte.ts` and `date-grid/dropPayload.ts` for pointer drag-and-drop of grid instances onto other grid days or the timeline) and `do/` (with `stopwatch/`, whose `engine.svelte.ts` consumes the generated `StopwatchState` — including the timed pause — plays the transition WAVs, and triggers the screen-effect overlay). `sidebar/state.svelte.ts` persists the sidebar's tab position/notes/visibility under `ui.sidebar.*`, seeding once from the legacy `sidebar_*` `localStorage` keys.
...
- `src-tauri/`: Tauri Rust backend.
  ...
  - `src-tauri/src/commands/`: IPC command handlers split by domain (`tasks.rs`, `events.rs`, `window.rs`, `sync.rs`). Every handler is a thin wrapper (argument marshalling + `db_pool(&app)`) over a pool-level function in the testable layer; the test-only `test_support/` module builds in-memory pools and row fixtures for the smoke tests in `commands/tests.rs` and `settings/tests.rs` (see `src-tauri/AGENTS.md` → Command Testing for the convention and why there is no mock-runtime test).
  - `src-tauri/src/error.rs`: `AppError` enum (`thiserror`) returned by ALL IPC commands; serialized as `{code, message}`.
  - `src-tauri/src/events.rs`: Centralized event-name constants (`STOPWATCH_UPDATED`, `SYNC_STARTED/FINISHED/ERROR`, `OAUTH_URL`, `DEEP_LINK`, `SCREEN_EFFECT`).
  - `src-tauri/src/db/`: Modularized SQLite operations using `sqlx` (`tasks.rs`, `events.rs`, `settings.rs`, `ui_state.rs`, `maintenance.rs` for the whole-mirror wipe, plus `task_filters.rs` for the dynamic `QueryBuilder` filtering path). `mod.rs` declares the submodules and `FilterColumnExt`; `migrations.rs` holds `init_db` (inline `CREATE TABLE IF NOT EXISTS` plus additive `ensure_column!` migrations so older databases gain `events.timezone`, `calendars.sync_token`, and `tasks.task_list_id`, then a transactional migration of legacy `ui.*` rows from `settings` into `ui_state`); backend CRUD tests live in `tests.rs`.
  ...
  - `src-tauri/src/stopwatch.rs`: Stopwatch backend (`StopwatchState` struct plus `get/toggle_break/toggle_pause/adjust_pause/reset_stopwatch` commands).
...
- **Event Name Constants**: Backend event names are constants in `src-tauri/src/events.rs`, mirrored in `src/lib/events.ts` (`stopwatch-updated`, `sync-started`, `sync-finished`, `sync-error`, `oauth-url`, `screen-effect`). Never inline raw event strings on either side.
...
- **Multi-Window Architecture**: Four windows are declared in `tauri.conf.json`:
  ...
  - **Effect Window** (`effect`): A hidden transparent, always-on-top, click-through fullscreen overlay that plays the phase-transition screen effect. `show_screen_effect`/`hide_screen_effect` drive it and the `SCREEN_EFFECT` event restarts its canvas animation.
  All of them render the same SPA, branched by window label in `+layout.svelte`.
- **Cross-Window State**: There is no dedicated inter-window wiring module. Window-specific state (stopwatch, sync status) lives in Rust and is synced across windows through Tauri commands (`get_stopwatch_state`, `toggle_stopwatch`, `toggle_break`, `toggle_pause`, `adjust_pause`, `reset_stopwatch`, `get_sync_state`) plus backend-emitted events consumed via `listen` with the `src/lib/events.ts` constants.
