...
- **Persisted UI State**: Per-component UI state (filters, sorts, date-grid view, timeline day count, split-pane sizes, Do-screen section collapse) is persisted with the `persistState` rune (`src/lib/persisted.svelte.ts`) under `ui.*` keys in the backend `settings` table. It hydrates once from `get_setting` and debounce-saves via `update_setting`; while hydration is in flight, writes are suppressed, and only values that differ from the stored/fallback value are written.
