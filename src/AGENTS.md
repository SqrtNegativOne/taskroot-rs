...
## Key Concepts
- **Hydration Primitive**: `src/lib/asyncState.svelte.ts` holds the shared pieces for reading backend state into runes: `createStaleGuard` (only the latest of overlapping requests may apply its response), `createDebouncedWriter` (coalesce rapid writes and `flush` a pending one on teardown), and `hydrateOnce` (read once on mount, skipping a response that is no longer current). `persistState` is composed from all three and `useTauriQuery` uses the stale guard; when adding backend-backed reactive state, compose these rather than hand-rolling another flag/timer/request-id state machine.
...
- **Persisted UI State**: Per-component UI state (filters, sorts, date-grid view, timeline day count, split-pane sizes, Do-screen section collapse) is persisted with the `persistState` rune (`src/lib/persisted.svelte.ts`) in the backend `ui_state` table via `set_ui_state`. It hydrates once from `get_ui_state` and debounce-saves via `set_ui_state`; while hydration is in flight, writes are suppressed, and only values that differ from the stored/fallback value are written. It is built on the shared primitives in `src/lib/asyncState.svelte.ts` (see Hydration Primitive above).
- **Persistence Channels**: Backend SQLite is the source of truth for anything that must survive a webview data reset or be shared across windows. Typed app settings are `AppSettings` fields written with `update_setting`; arbitrary per-component UI state goes through `get_ui_state`/`set_ui_state` in the `ui_state` table. `localStorage` is not used for user-facing state. (`src/routes/dev/+page.svelte` is the dev inspector and is exempt.)
- **Sidebar State**: The sidebar is a separate `sidebar` webview, so its state goes through the shared backend. `src/screens/sidebar/state.svelte.ts` (`createSidebarState`) persists the tab position, notes and notes-pane visibility under `ui.sidebar.*`; `readOrSeedSidebar` consults the legacy `sidebar_*` `localStorage` keys only when the backend has no row, writes the seed, then removes the legacy key (one-time migration). Notes/visibility write back through `createDebouncedWriter`; `saveTabTop` persists a finished drag.
...
- **Grid Drag & Drop**: `src/screens/plan/hooks/eventDrag.svelte.ts` owns the pointer gesture for dragging date-grid instances; it hit-tests `[data-drop-kind]` elements (`grid-day` cells and `day-time` columns) and stores the target in the shared `dragState` so `DayCell`/`DropPreview` can highlight it. `src/screens/plan/date-grid/dropPayload.ts` is the pure payload seam: a `grid-day` drop shifts the date (all-day stays all-day, timed keeps local time-of-day and duration), a `day-time` drop places the instance at a snapped minute (timed keeps duration, all-day defaults to 1 hour). Drops persist through `store.rescheduleEvent(id, startTime, endTime, isAllDay)` → `reschedule_event`.
